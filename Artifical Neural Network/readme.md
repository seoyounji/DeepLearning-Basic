...
AI는 Narrow AI와 General AI로 나뉘는데 General AI는 영화에서 나오는 것처럼 인간이 하는 모든 행동을 똑같이 할 수 있는 인공지능을 말하고 Narrow AI는 현재 산업에서 주로 다루는 인공지능을 말하는데 이 인공지능은 보통 한가지만 잘한다. 예를 들어 사람 얼굴을 찾는다던가, 이 사진이 개인지 고양이인지 판단한다던가 하는 일들.      

그렇다면 어떻게 이 AI를 학습시킬 수 있을까?  여기서 머신러닝이 등장한다.         

​             

머신러닝은 AI를 달성하기 위한 수단이다.       

기계를 학습시키는 데에는 크게 세가지 학습법이 있는데 Supervised learning, Unsupervised learning, Reinforcement learning 이다.      

이 3가지는 특히 중요하므로 다른 글에서 자세히 다룰 예정이다.        

아무튼 이렇게 기계를 학습시키는 방법들이 있는데 이 학습을 어떻게 진행할까?  여기서 딥러닝이 등장한다.       

​               

즉 딥러닝은 머신러닝을 달성하기 위한 수단이다.        

딥러닝은 인간의 뇌처럼 작동하는 알고리즘(Artificial Neural Network 라고 함) 을 기반으로 엄청나게 방대한 양의 데이터를 이용해 기계를 학습시킨다.        
...
이 Multi Layer Network, 즉 Deep Neural Network의 경우 컴퓨터가 스스로 특징을 찾아내고 데이터를 구분짓는 과정을 반복하여 최적의 결과를 도출해내게 되고 이를 응용한 알고리즘이 바로 CNN, RNN인 것이다.      
